...
import dev.foxgirl.rembulan.load.ChunkClassLoader
import dev.foxgirl.rembulan.runtime.LuaFunction
import dev.foxgirl.rembulan.test.Util
import dev.foxgirl.rembulan.test.fragments.BasicFragments
      ...
      describe (fragment.description) {
        ...
        it ("can be compiled to Java bytecode") {
          Util.silenced {
            ...
            withProperty("dev.foxgirl.rembulan.compiler.VerifyAndPrint", "true") {
        ...
        it ("can be loaded by the VM") {
          Util.silenced {
            ...
            val cm = withProperty("dev.foxgirl.rembulan.compiler.VerifyAndPrint", "true") {
              val compiler = new LuaCompiler(settings)

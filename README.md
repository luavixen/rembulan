...
Note that this repository is a fork of Rembulan with performance improvements, view the original repository [here](https://github.com/mjanicek/rembulan).

## About

Rembulan is an implementation of Lua 5.3 for the Java Virtual Machine (JVM), written in Java 7 with minimal dependencies. The goal of the Rembulan project is to develop a correct, complete and scalable Lua implementation for running sandboxed Lua programs on the JVM.

Rembulan implements Lua 5.3 as specified by the [Lua Reference Manual](http://www.lua.org/manual/5.3/manual.html), explicitly attempting to mimic the behaviour of PUC-Lua whenever possible. This includes language-level features (such as metamethods and coroutines) and the standard library.

## Status

The majority of language-level features are implemented, and can be expected to work. If you find behaviour that does not conform to Lua 5.3 as defined by the Lua Reference Manual, please [open a new issue](https://github.com/luavixen/rembulan/issues).

See also the [completeness table](doc/CompletenessTable.md) that maps out the current completeness status of Rembulan with regard to PUC-Lua, in particular the standard library.
...
To fetch the latest code on the `main` branch and build it:
...
To download the latest release artifact through Maven, first add my personal Maven repository to the repositories configuration to your `pom.xml`:
...
} catch (CallPausedException ignored) {
...
The [`CallPausedException`](https://luavixen.github.io/rembulan/apidocs/rembulan-runtime/dev/foxgirl/rembulan/exec/CallPausedException.html) contains a *continuation* of the call. The call can be resumed; the pause is transparent to the Lua code, and the loop does not end with an error (it is merely paused).
...
Rembulan is a multi-module Maven build, consisting of the following modules:
...
Rembulan is licensed under the Apache License Version 2.0. See the file [LICENSE](LICENSE) for details.

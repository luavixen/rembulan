...
import dev.foxgirl.rembulan.runtime.Coroutine;
 ...
 * an {@link ExecutionContext ExecutionContext}.</p>
 */
public class ArgumentIterator implements Iterator<Object> {
	...
	private Number peekNumber() {
		Object arg = peek(LuaFormat.TYPENAME_NUMBER.toString());
		Number n = Conversions.numericalValueOf(arg);
		if (n != null) {
			return n;
		}
		else {
			throw new UnexpectedArgumentException(LuaFormat.TYPENAME_NUMBER.toString(), namer.typeNameOf(arg).toString());
		}
	}
	...
	public ByteString nextString() {
		...
		try {
			Object arg = peek(LuaFormat.TYPENAME_STRING.toString());
			ByteString v = Conversions.stringValueOf(arg);
			if (v != null) {
				result = v;
			}
			else {
				throw new UnexpectedArgumentException(LuaFormat.TYPENAME_STRING.toString(), namer.typeNameOf(arg).toString());
			}
		}
		catch (RuntimeException ex) {
			throw badArgument(ex);
	 ...
	 * {@code o} to {@linkplain LuaType#isString(Object) <i>be</i> a string},
	...
	public LuaFunction nextFunction() {
		return nextStrict(LuaFormat.TYPENAME_FUNCTION, LuaFunction.class);
	}
	...
	public Table nextTable() {
		return nextStrict(LuaFormat.TYPENAME_TABLE, Table.class);
	}
	...
	public Coroutine nextCoroutine() {
		return nextStrict(LuaFormat.TYPENAME_THREAD, Coroutine.class);
	}
